...
BANNER = [
    ...
    "=== GRID ===",
    "Division : 10 cols x 20 rows  = 200 cells",
...
GOTO_OK = [
    "",
    "=== GOTO CELL [3,5] ===",
    "  Target position: X -1515 / Y 2125",
    2.0,
    "  ARRIVED at cell [3,5]  pos X -1515 / Y 2125",
]
...
rig, fake = fake_rig()
check("connect pushes the grid", "S 10 20" in fake.written, str(fake.written))
check("connect does not home", "0+" not in fake.written, str(fake.written))
check("READY grid captured", rig.ready_grid == "10x20", str(rig.ready_grid))
