...
# correction X = lround(BUILD_PLACEMENT_OFFSET_X(-0.4)*spc) + skew_x(0) + lround(dx*spc)
check("horizontal pick X correction sums the -0.4 cm placement offset and the nudge",
      hlx.correction_steps == _lround(-0.4 * HSPC_X) + _lround(0.5 * HSPC_X))
check("horizontal place X carries the placement offset even with no nudge",
      next(l for l in pf_h.legs if l.leg == "place" and l.axis == "x"
           ).correction_steps == _lround(-0.4 * HSPC_X))

# Far horizontal column [2] holder X = 17.1 - 0.9 = 16.2 cm, then -0.4 cm
# placement offset pulls it toward home — an inward outlier, still on travel.
...
# holder X = 1.9 - 0.9 = 1.0 cm (~200 steps); the -0.4 cm placement offset and
# a -1.0 cm inward nudge together drive it below the home switch.
pf_ho = preflight_correction(grid=HGRID, mode="horizontal",
                             pick_cell=(0, 4), place_cell=(0, 4),
                             dx_cm=-1.0, dy_cm=0.0, tool_offset_cm=HTOOL)
