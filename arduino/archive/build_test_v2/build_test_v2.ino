/*
  ...
  28BYJ-48 + ULN2003 stepper that jogs +/-180 degrees on command and
  rotates gradually during the BUILD descent at the target cell.

  NEW IN build_test_v2: the B (BUILD) command synchronizes claw
  rotation with the final Z descent at the requested build location.
  The block reaches its requested R/RR angle on the final Z step.

  The B command performs one full
  ...
  ------------------------------------------------------------
    ...
    3. Verify the claw is neutral (manual-rotation safety check)
    4. Open the claw
    ...
    9. Z down to the requested BLOCK LEVEL while rotating gradually
       if this build asked for R / RR; both motions finish together
   10. Open the claw                             (block is placed)

  ...then it PARKS itself, so the rig never sits over the stack with
  an open claw and the next B starts from a known state:

   11. Z up to the TOP SWITCH                    (clear of the block)
   12. Rotate the claw back to neutral at target (if this build turned it)
   13. X/Y home to the origin                    (already neutral)

  Phases 11-13 are skipped if BUILD_PARK_AFTER_PLACE is false. A
...
// Rotation calibration used by BOTH manual R/RR and synchronized
// BUILD descent. Tune this if a commanded 180 degrees is physically
// short or long; the DDA synchronizer always completes exactly this
// many auxiliary steps on the final Z descent step.
const long AUX_STEPPER_180_STEPS = AUX_STEPPER_STEPS_PER_REV / 2;
...
// just placed, un-rotate at the target, then walk X/Y to the origin.
//
...
const uint8_t BUILD_STEP_COUNT = 13;
...
// the NEXT pick-up. After release, the build raises Z and UN-ROTATES
// while still over the target. It then returns to the feeder already
// neutral. The requested rotation is applied only during descent at
// the target, never during pickup.
//
//   build did R   ->  phase 12 rotates RR at target (back to 0)
//   build did RR  ->  phase 12 rotates R at target  (back to 0)
//   build did NR  ->  phase 12 has nothing to undo

const int8_t ROT_NONE = 0;
const int8_t ROT_CW = +1;  // "R"
const int8_t ROT_CCW = -1; // "RR"
const int8_t ROT_PARTIAL = 2;

// Where the claw is RIGHT NOW, relative to neutral.
int8_t clawRotation = ROT_NONE;

// Exact signed auxiliary-motor offset from neutral. Unlike the
// three-state label above, this remains truthful during a partially
// completed synchronized move and after manual R/RR commands.
long clawRotationStepOffset = 0;
...
void refreshClawRotationState()
{
  if (clawRotationStepOffset == 0)
  {
    clawRotation = ROT_NONE;
  }
  else if (clawRotationStepOffset == AUX_STEPPER_180_STEPS)
  {
    clawRotation = ROT_CW;
  }
  else if (clawRotationStepOffset == -AUX_STEPPER_180_STEPS)
  ...
  {
    clawRotation = ROT_PARTIAL;
  }
}

// Lowest-level auxiliary motion primitive. Keeping both counters here
// makes an interrupted synchronized descent recoverable: the next
// rotateClawTo() knows the exact number of steps needed for neutral.
void moveAuxMotorSteps(long signedSteps)
{
  while (signedSteps != 0)
  {
    int chunk;
    if (signedSteps > 30000L)
    {
      chunk = 30000;
    }
    else if (signedSteps < -30000L)
    {
      chunk = -30000;
    }
    else
    {
      chunk = (int)signedSteps;
    }

    auxStepper.step(chunk);
    auxStepperPos += chunk;
    clawRotationStepOffset += chunk;
    signedSteps -= chunk;
  }

  refreshClawRotationState();
}

void stepAuxMotorOnce(int8_t sign)
{
  auxStepper.step((sign > 0) ? 1 : -1);
  auxStepperPos += sign;
  clawRotationStepOffset += sign;
}
...
void rotateAuxStepperCW()
{
  ...
  moveAuxMotorSteps(AUX_STEPPER_180_STEPS);
  statRotCW++;
...
// whatever it is doing now. The exact step offset is used so this can
// also recover from a partially completed synchronized descent.
void rotateClawTo(int8_t target)
{
  long targetSteps = (long)target * AUX_STEPPER_180_STEPS;
  long deltaSteps = targetSteps - clawRotationStepOffset;
  ...
  long magnitude = (deltaSteps > 0) ? deltaSteps : -deltaSteps;
  unsigned long calibratedMoves =
      (unsigned long)((magnitude + AUX_STEPPER_180_STEPS - 1) /
                      AUX_STEPPER_180_STEPS);

  moveAuxMotorSteps(deltaSteps);

  if (deltaSteps > 0)
  {
    statRotCW += calibratedMoves;
  }
  else
  {
    statRotCCW += calibratedMoves;
  }
...
// Descend Z and rotate the auxiliary gripper motor as one coordinated
// move. A DDA accumulator maps the full calibrated 180-degree motor
// move onto however many Z steps remain to THIS level. Consequently:
//
//   level 0     -> rotation is spread over the full top-to-ground drop
//   high levels -> the same rotation is spread over the shorter drop
//
// The final auxiliary step is issued immediately before the final Z
// pulse, so the gripper is fully rotated when the requested height is
// reached. No time/rpm estimate is involved; synchronization is based
// entirely on counted steps from both calibrated mechanisms.
bool zGoLevelWithRotation(long level, int8_t wantRot)
{
  if (wantRot == ROT_NONE)
  {
    return zGoLevel(level);
  }

  if (!axisHomed[AXIS_Z])
  {
    Serial.println(F("  !! Z has no zero - cannot synchronize descent."));
    return false;
  }

  long target = levelToZSteps(level);
  long zDelta = target - axisPos[AXIS_Z];

  if (zDelta > 0)
  {
    Serial.println(F("  !! synchronized BUILD move must descend, not rise."));
    return false;
  }

  unsigned long zTotal = (unsigned long)(-zDelta);
  long auxTarget = (long)wantRot * AUX_STEPPER_180_STEPS;
  long auxDelta = auxTarget - clawRotationStepOffset;
  int8_t auxSign = (auxDelta >= 0) ? DIR_POS : DIR_NEG;
  unsigned long auxTotal =
      (unsigned long)((auxDelta >= 0) ? auxDelta : -auxDelta);

  Serial.print(F("  Synchronized descent: "));
  Serial.print(zTotal);
  Serial.print(F(" Z steps + "));
  Serial.print(auxTotal);
  Serial.print(F(" rotation steps -> "));
  Serial.println(rotationName(wantRot));

  // A target exactly at the current carry height has no descent over
  // which to distribute rotation. Complete the calibrated rotation
  // here so arrival is still correct; normal configured build levels
  // are below the 26.5 cm top switch and do not take this branch.
  if (zTotal == 0)
  {
    Serial.println(F("  NOTE: zero Z travel available; rotating in place."));
    rotateClawTo(wantRot);
    return (level <= 0) ? zGoGround() : true;
  }

  int8_t moveIndex = findMoveIndex(AXIS_Z, DIR_NEG);
  if (moveIndex < 0)
  {
    Serial.println(F("  !! no configured Z- move for synchronized descent."));
    return false;
  }

  enableMotors();
  setDirection(MOVES[moveIndex].dir1,
               MOVES[moveIndex].dir2,
               MOVES[moveIndex].dir3);

  unsigned int pulseDelay = stepDelayOf(AXIS_Z);
  unsigned long zDone = 0;
  unsigned long auxDone = 0;

  for (unsigned long i = 0; i < zTotal; i++)
  {
    uint8_t blocked = blockReason(AXIS_Z, DIR_NEG);
    if (blocked != BLOCK_NONE)
    {
      stepCounts[moveIndex] += zDone;

      // The bottom switch is the requested destination for level 0.
      // Small switch hysteresis can make it trigger a pulse earlier
      // than the measured count; finish any tiny rotation remainder
      // at contact so the final physical pose is still exact.
      if (level <= 0 && blocked == BLOCK_PHYSICAL)
      {
        while (auxDone < auxTotal)
        {
          stepAuxMotorOnce(auxSign);
          auxDone++;
        }
        refreshClawRotationState();
        clawRotation = wantRot;

        unsigned long turns =
            (auxTotal + AUX_STEPPER_180_STEPS - 1) /
            AUX_STEPPER_180_STEPS;
        if (auxSign > 0)
          statRotCW += turns;
        else
        ...
        Serial.println(F("  Ground switch reached; synchronized rotation complete."));
        return true;
      }

      refreshClawRotationState();
      statShortMoves[AXIS_Z]++;
      Serial.println(F("  !! synchronized descent stopped before its target."));
      return false;
    }

    // Rounded cumulative DDA target for this Z step. On the final
    // iteration desiredAux is exactly auxTotal.
    unsigned long desiredAux =
        (((i + 1UL) * auxTotal) + (zTotal / 2UL)) / zTotal;

    while (auxDone < desiredAux)
    {
      stepAuxMotorOnce(auxSign);
      auxDone++;
    }
  ...
  if (level <= 0)
  {
    if (isPhysicalBlocked(AXIS_Z, DIR_NEG))
    {
      return true;
    }

    // The measured top-to-bottom count can differ by a few pulses
    // because of switch hysteresis. Rotation is already complete;
    // finish the short seek so the hardware switch defines exact zero.
    Serial.println(F("  Finishing the final ground-switch approach ..."));
    return zGoGround();
  }

  if (axisPos[AXIS_Z] != target)
  {
    Serial.println(F("  !! synchronized descent ended at the wrong Z count."));
    return false;
  }

  Serial.println(F("  Synchronized descent and rotation complete."));
  return true;
}
...
void printBuildUsage()
{
  ...
  Serial.println(F("    R/RR rotates gradually during the target Z descent"));
...
// ------------------------------------------------------------
// PHASES 11-13 - park the rig after the block is down.
// ------------------------------------------------------------
...
// it. Undo rotation while still high over the target, then home X/Y.
// This guarantees a BUILD-requested R/RR never rotates at the feeder.
//
...
bool buildPark()
{
  buildStep(11, "Raise Z clear of the block just placed");
  if (!zGoTop())
  {
  ...
  buildStep(12, "Return the claw to neutral while still at the target");
  rotateClawTo(ROT_NONE);
  buildPause();

  buildStep(13, "Return X/Y to the origin with the claw neutral");
  if (!goToOrigin())
  {
...
bool buildBlock(long col, long row, long level, int8_t wantRot)
{
  ...
  // Normally a no-op, because phase 12 already left the claw neutral
  // at the previous target. This only catches a manual R/RR issued
  // between builds; BUILD-requested rotation never occurs here.
  ...
  // ---- 9. descend and rotate together at the target cell ----

  buildStep(9, "Lower Z while gradually applying the requested rotation");
  if (!zGoLevelWithRotation(level, wantRot))
  {
    buildAbort("synchronized Z/rotation move did not reach the target");
    return false;
  }
  buildPause();
  ...
  buildStep(10, "Open the claw (release the block)");
  openServoAndWait();
  ...
  // ---- 11-13. park the rig back at its rest state ----
...
void printAuxStepperStatus()
{
  ...
  if (clawRotationStepOffset > 0)
    Serial.print(F("+"));
  Serial.print(clawRotationStepOffset);
  Serial.print(F(" steps)"));
  if (clawRotation != ROT_NONE)
  {
    Serial.print(F("  <- manual/partial offset; next B restores neutral"));
  }
  Serial.println();
}
...
void printBuildConfig()
{
  ...
  Serial.print(F("Rotation sync: "));
  Serial.print(AUX_STEPPER_180_STEPS);
  Serial.println(F(" aux steps / 180 deg, DDA-mapped across target descent"));
...
{
  Serial.println(F("======================================"));
  Serial.println(F("CNC Grid + BUILD v2 Control - MEGA 2560"));
  Serial.println(F("======================================"));
  ...
  Serial.println(F("    R/RR is synchronized with descent at the target cell"));
