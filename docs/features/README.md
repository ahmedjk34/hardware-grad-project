...
| [CORRECTION action](correction-action.md) | **BUILT on the Pi + UI; firmware `P` verb UNFLASHED / UNVERIFIED.** Vertical, level 0, axis-aligned, no taller neighbour, destination clear; `DISPLACED` also band-gated (0.5–1.2 cm — geometrically narrow). DESIGN.md §8 / D11 amended with the operator-initiated carve-out. Needs the two Stage-15 bench measurements before the band bounds are trusted | 4 / 5 | An operator presses `RETURN BLOCK TO CELL` on a `MOVED` / `DISPLACED` verdict; the server re-checks safety, drives Stage 15's `P` verb to grip the block where it is and set it on its planned cell, then re-verifies. `MOVED` is the clean case (grip on a real cell); `DISPLACED` is marginal. |
