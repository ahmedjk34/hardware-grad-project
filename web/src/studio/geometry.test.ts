...
describe("machine-space geometry", () => {
  ...
  it("measures footprint overlap in square millimetres", () => {
    ...
    // block's own 22 mm width, and its full 22 mm depth lands within the
    // vertical block's Y run, so the cross-mode contact is the whole 22 x 22 mm.
    // With horizontal's shipped registration (+1.9 cm trim + 0.5 / 0.3 cm error
    // offset) its [1,1] block sits over vertical column 3, not 2.
    const overlap = footprintOverlapArea(aabbOf(block("horizontal", 1, 1, 1)), aabbOf(block("vertical", 3, 1, 0)));
